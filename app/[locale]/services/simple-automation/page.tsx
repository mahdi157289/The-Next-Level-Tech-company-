...
import { CheckCircle2, Zap, Clock, ArrowRight, MousePointer2, Sparkles, Send, Layout, ShieldCheck, Activity, Link as LinkIcon, Settings } from 'lucide-react';
...
export default function SimpleAutomationServicePage() {
  const t = useTranslations('simpleAutomationService');
  ...
  const guideMessages = [
    "Phase 1: Auditing your manual workflows to identify hidden time-wasters.",
    "Phase 2: Building custom triggers and integrated logic between your apps.",
    "Phase 3: Launching your live sync engines to handle admin while you work."
  ];
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#FDFDFD] dark:bg-[#020817] transition-colors duration-700 selection:bg-emerald-500/30">
      <ParticlesBackground />
      <Navbar />
              ...
              <div className="sidebar-content lg:sticky lg:top-24 lg:h-[calc(100vh-8rem)] flex flex-col justify-center relative pl-14 pr-8 py-12 bg-white/50 dark:bg-white/[0.02] backdrop-blur-xl border-r border-gray-100 dark:border-white/5 shadow-2xl dark:shadow-none rounded-r-[3rem] lg:rounded-none overflow-hidden">
                ...
                <div 
                  ...
                  style={{
                    background: activePhase === 1 
                      ? 'radial-gradient(circle at 20% 50%, #10b981 0%, transparent 70%)'
                      : activePhase === 2
                      ? 'radial-gradient(circle at 20% 50%, #059669 0%, transparent 70%)'
                      : 'radial-gradient(circle at 20% 50%, #047857 0%, transparent 70%)'
                  }}
                />
                
                <div className="absolute left-6 top-24 bottom-24 w-[2px] bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                  <div 
                    className="w-full bg-gradient-to-b from-emerald-400 to-green-600 transition-all duration-300 ease-out rounded-full relative"
                    style={{ height: `${scrollProgress * 100}%` }}
                  >
                    <div className="indicator-glow absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-4 bg-emerald-400 rounded-full z-20" />
                  </div>
                </div>
                ...
                <div className="mb-10">
                  <div className="inline-flex items-center gap-2 text-emerald-500 mb-4">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em]">Efficiency Protocol</span>
                  </div>
                  <h2 className="text-3xl font-black text-gray-900 dark:text-white leading-tight mb-4">
                    {t('roadmap.title')}
                  </h2>
                  <div className="h-14 overflow-hidden">
                    <p key={activePhase} className="text-sm font-medium text-gray-500 dark:text-emerald-400/60 italic leading-relaxed animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                <nav className="space-y-8 relative">
                  <div 
                    className="absolute left-6 top-0 w-[2px] bg-emerald-500 transition-all duration-500 ease-out hidden lg:block"
                    style={{ 
                      height: '48px', 
                  ...
                  {[1, 2, 3].map((i) => (
                    ...
                    >
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-black transition-all duration-500 border-2 relative z-10 ${
                        activePhase === i 
                          ? 'bg-emerald-500 border-emerald-400 text-white shadow-xl shadow-emerald-500/20 scale-110' 
                          : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-400'
                      }`}>
                        {i}
                        {activePhase === i && (
                          <div className="absolute -inset-2 bg-emerald-500/20 rounded-[1.5rem] animate-pulse -z-10" />
                        )}
                      </div>
                      <div className="flex-1">
                        <p className={`text-[9px] font-black uppercase tracking-widest mb-1 transition-colors ${
                          activePhase === i ? 'text-emerald-500' : 'text-gray-400'
                        }`}>
                    ...
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="flex gap-4 group">
                        <div className="relative shrink-0">
                          <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center group-hover:scale-110 group-hover:bg-emerald-500/20 transition-all duration-500 border border-emerald-500/20">
                            <CheckCircle2 className="h-4 w-4 text-emerald-500" />
                          </div>
                          <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-1 h-1 rounded-full bg-emerald-500/40" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <h4 className="text-base font-black text-gray-900 dark:text-white group-hover:text-emerald-500 transition-colors leading-none">
              ...
              <section id="phase-2">
                <div className="max-w-3xl">
                  ...
                  <div className="reveal-item relative group">
                    <div className="absolute -inset-1 bg-gradient-to-r from-emerald-500 via-green-400 to-teal-600 rounded-[2.5rem] blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
                    <div className="relative bg-white dark:bg-[#080c14] border border-gray-200 dark:border-white/10 rounded-[2.5rem] p-10 md:p-14 shadow-2xl">
                      ...
                      <textarea 
                        className="w-full bg-gray-50 dark:bg-white/[0.03] border-2 border-transparent focus:border-emerald-500/30 rounded-3xl p-8 text-lg text-gray-900 dark:text-white placeholder:text-gray-300 dark:placeholder:text-white/10 min-h-[220px] resize-none transition-all outline-none"
              ...
              <section id="phase-3">
                <div className="max-w-4xl">
                  ...
                  <div className="reveal-item rounded-[3rem] bg-gray-900 dark:bg-black p-1 md:p-2 shadow-3xl relative overflow-hidden border border-white/5 group">
                    <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
