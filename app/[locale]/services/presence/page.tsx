...
import { CheckCircle2, Zap, Clock, ArrowRight, MousePointer2, Sparkles, Send, Layout, ShieldCheck } from 'lucide-react';
...
export default function PresenceServicePage() {
  const t = useTranslations('presenceService');
  ...
  const guideMessages = [
    "Phase 1: Establishing your professional baseline and digital authority.",
    "Phase 2: Crafting the aesthetic details that bring your brand to life.",
    "Phase 3: Architecting the strategy and functional map of your vision."
  ];
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // 2. Unified Scroll Tracking Logic for Phases
      [1, 2, 3].forEach((i) => {
        ScrollTrigger.create({
          trigger: `#phase-${i}`,
          start: 'top 50%',
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#FDFDFD] dark:bg-[#020817] transition-colors duration-700 selection:bg-teal-500/30">
      <ParticlesBackground />
      <Navbar />
          ...
          <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 relative">
            ...
            <aside className="lg:w-1/3 order-2 lg:order-1 mb-20 lg:mb-0">
              <div className="sidebar-content lg:sticky lg:top-24 lg:h-[calc(100vh-8rem)] flex flex-col justify-center relative pl-14 pr-8 py-12 bg-white/50 dark:bg-white/[0.02] backdrop-blur-xl border-r border-gray-100 dark:border-white/5 shadow-2xl dark:shadow-none rounded-r-[3rem] lg:rounded-none overflow-hidden">
                ...
                {/* Dynamic Phase Background Glow */}
                <div 
                  className="absolute inset-0 opacity-20 transition-all duration-1000 ease-in-out -z-10"
                  style={{
                    background: activePhase === 1 
                      ? 'radial-gradient(circle at 20% 50%, #14b8a6 0%, transparent 70%)'
                      : activePhase === 2
                      ? 'radial-gradient(circle at 20% 50%, #a855f7 0%, transparent 70%)'
                ...
                <div className="absolute left-6 top-24 bottom-24 w-[2px] bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                  <div 
                    className="w-full bg-gradient-to-b from-teal-400 to-teal-600 transition-all duration-300 ease-out rounded-full relative"
                    style={{ height: `${scrollProgress * 100}%` }}
                  >
                    <div className="indicator-glow absolute bottom-0 left-1/2 -translate-x-1/2 w-4 h-4 bg-teal-400 rounded-full z-20" />
                  </div>
                </div>
                ...
                <div className="mb-10">
                  <div className="inline-flex items-center gap-2 text-teal-500 mb-4">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em]">Protocol Guide</span>
                  </div>
                  <h2 className="text-3xl font-black text-gray-900 dark:text-white leading-tight mb-4">
                    {t('roadmap.title')}
                  </h2>
                  <div className="h-14 overflow-hidden">
                    <p key={activePhase} className="text-sm font-medium text-gray-500 dark:text-teal-400/60 italic leading-relaxed animate-in fade-in slide-in-from-bottom-2 duration-700">
                ...
                <nav className="space-y-8 relative">
                  ...
                  <div 
                    className="absolute left-6 top-0 w-[2px] bg-teal-500 transition-all duration-500 ease-out hidden lg:block"
                    style={{ 
                      height: '48px', 
                  ...
                  {[1, 2, 3].map((i) => (
                    ...
                    >
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-black transition-all duration-500 border-2 relative z-10 ${
                        activePhase === i 
                          ? 'bg-teal-500 border-teal-400 text-white shadow-xl shadow-teal-500/20 scale-110' 
                          : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-400'
                      }`}>
                        {i}
                        {activePhase === i && (
                          <div className="absolute -inset-2 bg-teal-500/20 rounded-[1.5rem] animate-pulse -z-10" />
                        )}
                      </div>
                      <div className="flex-1">
                        <p className={`text-[9px] font-black uppercase tracking-widest mb-1 transition-colors ${
                          activePhase === i ? 'text-teal-500' : 'text-gray-400'
                        }`}>
                ...
                <div className="mt-12 pt-8 border-t border-gray-100 dark:border-white/5 flex items-center justify-between">
                  <div className="flex items-center gap-2 text-gray-400 dark:text-white/20">
                    ...
                    <span className="text-[9px] font-black uppercase tracking-widest">7 Day Delivery</span>
                  </div>
                    ...
                    {[1, 2, 3, 4, 5, 6].map((i) => (
                      <div key={i} className="flex gap-4 group">
                        <div className="relative shrink-0">
                          <div className="w-10 h-10 rounded-xl bg-teal-500/10 flex items-center justify-center group-hover:scale-110 group-hover:bg-teal-500/20 transition-all duration-500 border border-teal-500/20">
                            <CheckCircle2 className="h-4 w-4 text-teal-500" />
                          </div>
                          {/* Decorative Connector Dot */}
                          <div className="absolute -left-3 top-1/2 -translate-y-1/2 w-1 h-1 rounded-full bg-teal-500/40" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <h4 className="text-base font-black text-gray-900 dark:text-white group-hover:text-teal-500 transition-colors leading-none">
              ...
              <section id="phase-2">
                <div className="max-w-3xl">
                  ...
                  <div className="reveal-item relative group">
                    <div className="absolute -inset-1 bg-gradient-to-r from-teal-500 via-emerald-400 to-purple-600 rounded-[2.5rem] blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
                    <div className="relative bg-white dark:bg-[#080c14] border border-gray-200 dark:border-white/10 rounded-[2.5rem] p-10 md:p-14 shadow-2xl">
                      ...
                      <textarea 
                        className="w-full bg-gray-50 dark:bg-white/[0.03] border-2 border-transparent focus:border-teal-500/30 rounded-3xl p-8 text-lg text-gray-900 dark:text-white placeholder:text-gray-300 dark:placeholder:text-white/10 min-h-[220px] resize-none transition-all outline-none"
              ...
              <section id="phase-3">
                <div className="max-w-4xl">
                  ...
                  <div className="reveal-item rounded-[3rem] bg-gray-900 dark:bg-black p-1 md:p-2 shadow-3xl relative overflow-hidden border border-white/5 group">
                    <div className="absolute inset-0 bg-gradient-to-br from-teal-500/20 to-purple-500/20 opacity-0 group-hover:opacity-100 transition-opacity duration-1000" />
